import { shouldUseNctracks, submitNcClaim, indexAck277ByPcn, nctracksPollIntervalMs, dollarsToCents, isRemittancePayable, getNctracksIntegrationStatus, hasRejectedInlineAck } from './nctracks';
...
describe('shouldUseNctracks', () => {
  ...
  it('skips NC commercial claims without NC Medicaid payer context', () => {
    expect(shouldUseNctracks('NC', 'COMMERCIAL_PLAN')).toBe(false);
  });
...
describe('submitNcClaim', () => {
  ...
  it('rejects placeholder recipient IDs before submission', async () => {
...
describe('hasRejectedInlineAck', () => {
  it('detects rejected 999 and 277CA acknowledgments', () => {
    expect(hasRejectedInlineAck({
      ack999: { accepted: false, errors: [], raw: 'AK9*R~' },
    })).toBe(true);
    expect(hasRejectedInlineAck({
      ack277CA: {
        status: 'rejected',
        perClaim: [{ patientControlNumber: 'PCN', status: 'rejected', categoryCode: 'A7', statusCode: '21' }],
        raw: 'STC*A7:21~',
      },
    })).toBe(true);
  });

  it('allows accepted acknowledgments', () => {
    expect(hasRejectedInlineAck({
      ack999: { accepted: true, errors: [], raw: 'AK9*A~' },
      ack277CA: {
        status: 'accepted',
        perClaim: [{ patientControlNumber: 'PCN', status: 'accepted', categoryCode: 'A0', statusCode: '20' }],
        raw: 'STC*A0:20~',
      },
    })).toBe(false);
  });
