...
describe('shouldUseNctracks', () => {
  ...
  it('does not route unknown NC payers through NCTracks', () => {
    expect(shouldUseNctracks('NC', 'AETNA_COMMERCIAL')).toBe(false);
  });
...
describe('submitNcClaim', () => {
  ...
  it('rejects placeholder Medicaid recipient IDs before submission', async () => {
    await expect(submitNcClaim({
      ccn: 'CCN-TEST-UNKNOWN',
      totalCharge: 125.5,
  ...
  it('rejects inline NCTracks negative acknowledgments', async () => {
    await expect(submitNcClaim({
      ccn: 'CCN-TEST-REJECT',
      totalCharge: 125.5,
      patientMedicaidId: 'NCMD00100001',
    ...
    })).rejects.toThrow('NCTracks rejected 837P functional acknowledgment');
  });
});
