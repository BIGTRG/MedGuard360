...
export function parse999(payload: string): Ack999 {
  ...
  for (const seg of segments) {
    const p = seg.split('*');
    if (p[0] === 'ISA') {
      interchangeControlNumber = p[13]?.trim();
    } else if (p[0] === 'AK1') {
      functionalGroupControlNumber = p[2]?.trim();
    } else if (p[0] === 'AK9') {
      accepted = p[1] === 'A';
  ...
  return { accepted, errors, interchangeControlNumber, functionalGroupControlNumber, raw: payload };
}
